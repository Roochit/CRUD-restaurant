...
class TestController extends Controller
{
    ...
    public function create(Request $request)
    {
        ...
        $messages = [
            ...
            'name3.required' => 'กรุณากรอกข้อมูล',
            'name3.min' => 'กรอกข้อมูลขั้นต่ำ :min ตัวอักษร',
            
        ];

        //rule 
        $validator = Validator::make($request->all(), [
            'name' => 'required|min:3|unique:test',
            'name2' => 'required|min:3',
            'name3' => 'required|min:3',
        ], $messages);
        ...
        try {
            ...
            TestModel::create([
                ...
                'name2' => strip_tags($request->input('name2')),
                'name3' => strip_tags($request->input('name3')),
            ]);
 ...
 public function edit($id)
    {
        try {
            ...
            if (isset($test)) {
                ...
                return view('test.edit', compact('id', 'name' ,'name2','name3'));
            }
        } catch (\Exception $e) {
            //return response()->json(['error' => $e->getMessage()], 500); //สำหรับ debug
            return view('errors.404');
 ...
 public function update($id, Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            'name' => [
                    'required',
                    'min:3',
                        Rule::unique('test', 'name','name2','name3')->ignore($id, 'id'), //ห้ามแก้ซ้ำ
            ],
    ], $messages);
